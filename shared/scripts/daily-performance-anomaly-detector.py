...
Runs every morning at 9:00 AM to analyze yesterday's performance.
...
def fetch_daily_performance(client, customer_id, target_date, baseline_start, baseline_end):
    """Fetch yesterday's performance vs 7-day baseline"""
    try:
...
def detect_anomalies(client_name, target_data, baseline_data):
    """Detect anomalies in yesterday's performance"""
    ...
    if target_data['revenue'] == 0 and baseline_revenue > THRESHOLDS['critical']['zero_revenue_min']:
        anomalies.append({
            'severity': 'critical',
            'type': 'zero_revenue',
            'message': f"Zero revenue yesterday (baseline: £{int(baseline_revenue)}/day)",
            'target_value': 0,
...
def create_anomaly_email(anomalies_by_client, target_date):
    ...
    <body>
        ...
        <p>Unusual performance patterns detected for yesterday's data.</p>
    """
    ...
    html += f"""
        <div class="footer">
            ...
            <p>Baseline: 7-day average (excluding yesterday)</p>
        </div>
    </body>
    </html>
    """
...
def main():
    ...
    baseline_end = target_date - timedelta(days=1)  # Day before yesterday
