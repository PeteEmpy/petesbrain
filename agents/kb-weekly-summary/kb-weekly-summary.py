...
def analyze_knowledge_base_with_claude(ai_emails, kb_documents, upcoming_tasks, performance_data=None, strategy_data=None, meeting_imports=None, launchagent_status=None, resource_allocation=None):
    ...
    if strategy_data and strategy_data.get('weekend_plans'):
        ...
        weekend_context = f"""
**⚠️ WEEKEND PLANS DETECTED**: {plans.get('summary', 'Weekend plans')}
...
"""
    ...
    if performance_data and performance_data.get('clients'):
        ...
        for client_data in performance_data['clients'][:12]:  # All 12 clients
            ...
            if changes['trend'] == 'up':
                perf_line += f" (↑ {abs(int(changes['revenue_pct']))}% WoW)"
    ...
    if meeting_imports and meeting_imports.get('total', 0) > 0:
        for client, meetings in sorted(meeting_imports['grouped'].items()):
            ...
            for meeting in meetings[:3]:  # Show up to 3 per client
                meeting_summaries.append(f"  • {meeting['filename']} ({meeting['time_display']})")

    # Prepare LaunchAgent status summaries
    agent_summaries = []
    if launchagent_status:
        if 'error' in launchagent_status:
            agent_summaries.append(f"⚠️ Could not check agent status: {launchagent_status['error']}")
        else:
            ...
            if healthy:
                agent_summaries.append(f"✅ All {total} LaunchAgents healthy")
            else:
                agent_summaries.append(f"⚠️ {failed}/{total} LaunchAgents failed:")
            ...
            for issue in launchagent_status.get('issues', []):
                agent_summaries.append(f"⚠️ {issue}")
    ...
    if resource_allocation:
        ...
        if resource_allocation.get('no_tasks'):
            allocation_summaries.append("**⚠️ CLIENTS WITH NO ACTIVE TASKS:**")
        ...
        if resource_allocation.get('over_allocated'):
            allocation_summaries.append("**🔴 OVER-ALLOCATED CLIENTS** (receiving disproportionate time):")
        ...
        if resource_allocation.get('under_allocated'):
            allocation_summaries.append("**🔴 UNDER-ALLOCATED CLIENTS** (not receiving enough time):")
...
{chr(10).join(allocation_summaries) if allocation_summaries else '✅ All clients have proportional time allocation'}

Please create a CONCISE, FORWARD-LOOKING weekly summary (aim for ~2-3 pages max) with these sections:

0. **⚠️ WEEKEND PLANS NOTICE** (if weekend plans detected)
...
1. **🎯 Strategic Priorities for This Week** - START HERE (Most Important!)
...
3. **🚨 Client Attention Required** - NEW! (Urgent actions this week)
...
4. **📊 Performance Context** (What happened last week to inform this week)
   - Group by trend: Strong Performers (↑), Stable (→), Needs Attention (↓)
...
6. **📝 Context Notes** (Background info - COLLAPSED/MINIMAL)
...
def create_html_email(summary_content):
    ...
    <body>
        ...
        <div class="footer">
            ...
            <p><em>📰 Industry news and AI updates are sent separately in the Weekly News Digest email.</em></p>
        </div>
    </body>
    </html>
    """
...
def main():
    ...
    log_message("📚 Gathering knowledge base documents...")
    kb_documents = []
    ...
    try:
        ...
        if result.returncode != 0:
            log_message(f"  ⚠️  Strategy generator had issues: {result.stderr}")
        else:
            log_message("  ✓ Weekly strategy generated successfully")
    except Exception as e:
        log_message(f"  ⚠️  Error running strategy generator: {e}")

    # Get client performance data
    log_message("📊 Loading client performance data...")
    performance_data = get_client_performance()
    if performance_data:
        log_message(f"  Loaded performance for {len(performance_data.get('clients', []))} clients")
    else:
        log_message("  ⚠️  No performance data available (may not have run yet)")

    # Get weekly strategic priorities
    log_message("🎯 Loading weekly strategic priorities...")
    ...
    if strategy_data:
        ...
        log_message("  ⚠️  No strategy data available")

    # Get Granola meeting imports
    log_message("📝 Loading Granola meeting imports...")
    ...
    if meeting_imports:
        ...
        if meeting_imports.get('has_unassigned'):
            log_message(f"  ⚠️  Some meetings are unassigned - require manual review")
    else:
        log_message("  No meeting imports found")

    # Check LaunchAgent status
    log_message("🏠 Checking LaunchAgent status...")
    launchagent_status = get_launchagent_status()
    if launchagent_status:
        if 'error' in launchagent_status:
            log_message(f"  ⚠️  Could not check agents: {launchagent_status['error']}")
        elif launchagent_status.get('healthy'):
            log_message(f"  ✓ All {launchagent_status.get('total_agents', 0)} agents healthy")
        else:
            log_message(f"  ⚠️  {launchagent_status.get('failed_count', 0)} agents with issues")

    # Check resource allocation (revenue vs time)
    log_message("💰 Analyzing resource allocation...")
    ...
    if resource_allocation:
        ...
        log_message(f"  ⚠️  Resource imbalances detected: {', '.join(issues)}")
    else:
        log_message("  ✓ Resource allocation is balanced")

    if len(kb_documents) == 0 and len(upcoming_tasks) == 0 and not performance_data and not strategy_data:
        log_message("⚠️  No new content or upcoming tasks. Skipping email.")
        return 0

    # Analyze with Claude
    log_message("🤖 Analyzing content with Claude API...")
    ...
    # Create HTML email
    log_message("📝 Creating HTML email...")
    ...
    subject = f"📊 Weekly Business Summary & Week Ahead - {today.strftime('%b %d')} to {week_ahead_end.strftime('%b %d')}"
