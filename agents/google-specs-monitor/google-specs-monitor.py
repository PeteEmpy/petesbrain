#!/usr/bin/env python3
"""
Google Specifications Monitor

Automatically checks Google Ads and GA4 documentation for updates weekly.
...
CONTENT_HASHES_FILE = PROJECT_ROOT / "data/cache/google-specs-content-hashes.json"
CHANGELOG_FILE = PROJECT_ROOT / "data/cache/google-specs-changelog.json"
STATE_FILE = PROJECT_ROOT / "data/state/google-specs-state.json"
LOG_FILE = PROJECT_ROOT / "data/cache/google-specs-monitor.log"
...
def batch_extract_specs(pages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    prompt = f"""Analyze these Google Ads/GA4 documentation pages and extract specifications and requirements.
...
Focus on:
- Asset requirements (headlines, descriptions, images, videos)
- Feed specifications (Merchant Center, product data)
- Campaign structure requirements
- Bidding strategy requirements
- Event/conversion specifications (for GA4)
- Dimension/metric specifications (for GA4)
...
[
    {{
        ...
        "url": "page URL",
        "specifications": {{
            "category": "google-ads|ga4",
            "subcategory": "asset-groups|merchant-center|events|etc",
            "specs": {{
...
def main():
    ...
    # Collect all URLs
    all_urls = []
    all_urls.extend(url_config.get("google_ads", []))
    all_urls.extend(url_config.get("ga4", []))
