"""
Google Chat OAuth Authentication - integrated into tool calls
"""
...
# Using sensitive scopes for full read/write access
SCOPES = [
...
# Environment variable for OAuth configuration
GOOGLE_CHAT_OAUTH_CONFIG_PATH = os.environ.get("GOOGLE_CHAT_OAUTH_CONFIG_PATH")

def get_oauth_credentials():
    """Get and refresh OAuth user credentials for Google Chat."""
    if not GOOGLE_CHAT_OAUTH_CONFIG_PATH:
        raise ValueError(
            "GOOGLE_CHAT_OAUTH_CONFIG_PATH environment variable not set. "
            "Please set it to point to your OAuth credentials JSON file."
        )

    if not os.path.exists(GOOGLE_CHAT_OAUTH_CONFIG_PATH):
        raise FileNotFoundError(f"OAuth config file not found: {GOOGLE_CHAT_OAUTH_CONFIG_PATH}")
    ...
    if not creds or not creds.valid:
        ...
        if not creds:
            ...
            try:
                ...
                with open(GOOGLE_CHAT_OAUTH_CONFIG_PATH, 'r') as f:
...
def get_headers_with_auto_token() -> Dict[str, str]:
    """Get API headers with automatically managed token - integrated OAuth for Google Chat."""
