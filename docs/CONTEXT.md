...
## ⚠️ CRITICAL: Google Ads API Data Accuracy Protocols

**When querying financial data (spend, revenue, conversions) for ANY client:**

### 1. Always Use Customer-Level Queries

```python
# CORRECT - Customer-level query
mcp__google-ads__run_gaql(
    customer_id='8573235780',
    manager_id='2569949686',  # Always include manager_id
    query='''
        SELECT
            metrics.cost_micros,
            metrics.conversions_value
        FROM customer
        WHERE segments.date BETWEEN '2025-11-03' AND '2025-11-25'
    '''
)

# WRONG - Campaign-level queries can miss data
# DO NOT USE: FROM campaign
```

### 2. Always Query Both Spend AND Revenue Together

Never make assumptions about revenue. If you don't have actual data, query it first.

### 3. Always Include manager_id

Required for all managed accounts. Check each client's CONTEXT.md for their manager account ID.

### 4. Query All Regional Accounts

For multi-account clients (e.g., Smythson: UK, USA, EUR, ROW), query all accounts and sum totals.

### 5. Always Present Raw API Data First

Before building projections or strategies, show the user the actual figures from the API.

### 6. Currency Reporting

Check each client's CONTEXT.md for currency reporting standards. Some clients (e.g., Smythson) require all figures in GBP regardless of account currency.

### Why This Matters

Campaign-level queries can miss spend/revenue. Customer-level queries are the source of truth for account totals. **Incorrect revenue data leads to incorrect strategic decisions.**
