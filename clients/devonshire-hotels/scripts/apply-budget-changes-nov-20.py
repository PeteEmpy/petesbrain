...
Implements balanced budget strategy:
- Restore client-approved budgets (The Fell, Dev Arms, Cavendish)
- Deploy remaining budget to high-ROAS campaigns
- Reduce underperformers

Uses Google Ads Python client library (same as MCP server).
"""
...
env_file = Path(__file__).parent.parent.parent.parent / 'infrastructure' / 'mcp-servers' / 'google-ads-mcp-server' / '.env'
if env_file.exists():
...
def update_budget(client, customer_id, budget_id, new_amount_micros):
    """Update a campaign budget using Google Ads Python client"""
...
def main():
    print("=" * 80)
    print("DEVONSHIRE HOTELS - BALANCED BUDGET IMPLEMENTATION")
    print("November 20, 2025")
    print("=" * 80)
    ...
    if successful == len(BUDGET_CHANGES):
        ...
        print("  • Dev Arms Hotel: +£48/day beyond approved (total £100/day)")
        ...
        print("  • Additional Revenue: +£827 from reallocation (10 days)")
        print("  • ROAS Improvement: 553% → 612% (blended)")
        print()
        print("NEXT STEPS:")
        ...
        print("  5. Log to experiment tracker and tasks-completed.md")
    else:
        print()
