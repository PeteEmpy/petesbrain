...
def query_period(client, start_date, end_date, period_name, conversion_filter=""):
    """Query data for a specific period"""

    # Build query - for 2024 we filter for web purchase only
    if conversion_filter:
        query = f"""
            ...
            WHERE
                ...
                AND segments.conversion_action_name = 'Website purchase'
        """
    else:
        query = f"""
            SELECT
    ...
    brand_data = {'spend': 0, 'conversions': 0, 'revenue': 0, 'clicks': 0}
    nonbrand_data = {'spend': 0, 'conversions': 0, 'revenue': 0, 'clicks': 0}

    print(f"Querying {period_name} data ({start_date} to {end_date})...")
...
def main():
    ...
    this_year_end = today - timedelta(days=1)  # Yesterday
    this_year_start = this_year_end - timedelta(days=29)  # 30 days total
    ...
    print(f"\nNote: Last year data filtered for 'Website purchase' conversion action only\n")

    # Query both periods
    brand_2025, nonbrand_2025 = query_period(
        client,
    ...
    brand_2024, nonbrand_2024 = query_period(
        client,
        last_year_start.strftime('%Y-%m-%d'),
        last_year_end.strftime('%Y-%m-%d'),
        "2024",
        conversion_filter="web_purchase"
    )
    ...
    cpc_increase = ((brand_2025_cpc - brand_2024_cpc) / brand_2024_cpc * 100) if brand_2024_cpc > 0 else 0
    print(f"\n1. BRAND CPC CHANGE: {format_change(brand_2024_cpc, brand_2025_cpc)}")
    ...
    roas_change = ((brand_2025_roas - brand_2024_roas) / brand_2024_roas * 100) if brand_2024_roas > 0 else 0
    ...
    print(f"\n4. SPEND EFFICIENCY:")
    spend_increase = ((brand_2025['spend'] - brand_2024['spend']) / brand_2024['spend'] * 100) if brand_2024['spend'] > 0 else 0
    revenue_increase = ((brand_2025['revenue'] - brand_2024['revenue']) / brand_2024['revenue'] * 100) if brand_2024['revenue'] > 0 else 0
    ...
    if spend_increase > revenue_increase:
        print(f"   ⚠️  Spend growing faster than revenue - confirming reduced efficiency")
