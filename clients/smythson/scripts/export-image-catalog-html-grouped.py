...
"""
Export Smythson image asset catalog to browsable HTML page with category grouping.

Creates an HTML page with images organized by detected categories (product types,
campaigns, etc.) based on asset naming patterns.

Usage:
    python3 export-image-catalog-html-grouped.py --region uk
    python3 export-image-catalog-html-grouped.py --region row
"""
...
def categorize_asset(name: str) -> str:
    ...
    if any(word in name_lower for word in ['bag', 'bags', 'tote', 'clutch', 'briefcase']):
        return 'Bags'
    elif any(word in name_lower for word in ['notebook', 'notebooks', 'diary', 'diaries', 'journal']):
        ...
        return 'Home & Office'
    elif any(word in name_lower for word in ['tech', 'ipad', 'iphone', 'airpod', 'device']):
        return 'Tech Accessories'
    elif any(word in name_lower for word in ['gift', 'stocking', 'xmas', 'christmas', 'holiday']):
        return 'Gifts & Seasonal'
    elif any(word in name_lower for word in ['black friday', 'black_friday', 'bf_', 'blackfriday']):
        return 'Black Friday'
    elif any(word in name_lower for word in ['ss20', 'ss21', 'ss22', 'aw20', 'aw21', 'aw22']):
...
def generate_grouped_html(region_name: str, assets: List[Dict], usage_map: Dict[str, List[str]]) -> str:
...
"""
    ...
    for category_name, category_assets in sorted_categories:
        category_id = category_name.replace(' ', '-').replace('\'', '').lower()
...
def export_region_catalog(region: str):
    ...
    html = generate_grouped_html(region_name, assets, usage_map)
    ...
    filename = f"image-catalog-{region}-grouped-{timestamp}.html"
...
def main():
    parser = argparse.ArgumentParser(
        description='Export Smythson image asset catalog with category grouping to HTML page'
    )
    parser.add_argument(
        '--region',
        required=True,
    ...
    print(f"\nSmythson Image Catalog HTML Export Tool (Grouped)")
