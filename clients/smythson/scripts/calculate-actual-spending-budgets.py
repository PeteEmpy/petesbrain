#!/usr/bin/env python3
"""
Calculate total daily budgets for campaigns that ACTUALLY SPENT in last 7 days
Based on actual spend data from Dec 15-21, 2025
"""

# UK Account - Campaigns with actual spend (Dec 15-21)
uk_spending_budgets = {
...
# USA Account - Campaigns with actual spend
usa_spending_budgets = {
...
# EUR Account - Campaigns with actual spend
eur_spending_budgets = {
...
# ROW Account - Campaigns with actual spend
row_spending_budgets = {
...
def calculate_unique_budgets(campaigns):
    """Calculate total budget from campaigns, handling shared budget pools"""
    unique_budgets = {}
    for campaign_id, (budget_id, amount_micros, name) in campaigns.items():
...
uk_unique = calculate_unique_budgets(uk_spending_budgets)
usa_unique = calculate_unique_budgets(usa_spending_budgets)
eur_unique = calculate_unique_budgets(eur_spending_budgets)
row_unique = calculate_unique_budgets(row_spending_budgets)
...
print("=" * 80)
print("ACTUAL DAILY BUDGET - CAMPAIGNS WITH SPEND (Dec 15-21)")
print("=" * 80)
print()
print(f"UK Account:   £{uk_total_gbp:,.2f}  ({len(uk_unique)} unique budget pools, {len(uk_spending_budgets)} campaigns)")
print(f"USA Account:  £{usa_total_gbp:,.2f}  ({len(usa_unique)} unique budget pools, {len(usa_spending_budgets)} campaigns)")
print(f"EUR Account:  £{eur_total_gbp:,.2f}  ({len(eur_unique)} unique budget pools, {len(eur_spending_budgets)} campaigns)")
print(f"ROW Account:  £{row_total_gbp:,.2f}  ({len(row_unique)} unique budget pools, {len(row_spending_budgets)} campaigns)")
print()
print("-" * 80)
