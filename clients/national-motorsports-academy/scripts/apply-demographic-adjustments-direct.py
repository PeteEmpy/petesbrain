#!/usr/bin/env python3
"""
NMA Demographic Bid Adjustments - Direct Google Ads API Tool
Uses the authentication from PetesBrain's MCP infrastructure
...
Setup:
    ...
    2. Google Ads OAuth credentials are auto-managed via ~/.config/google-ads-api/

Usage:
    python3 apply-demographic-adjustments-direct.py [--dry-run] [--confirm]
...
def get_google_ads_client():
    """Initialize Google Ads client from ~/.config/google-ads-api/"""
    try:
        # Try loading from default location ~/.config/google-ads-api/
        client = GoogleAdsClient.load_from_storage(
            version="v22",
            path_to_config=os.path.expanduser("~/.config/google-ads-api/")
        )
        return client
    except Exception as e:
        print(f"Note: Could not load from ~/.config/google-ads-api/: {e}")
        ...
        try:
            ...
            print(f"Error: Could not initialize Google Ads client: {e2}")
            print("\nPlease ensure you have authenticated with Google Ads API first.")
            print("If you have not, please run: google-ads-oauth-tool")
            sys.exit(1)
